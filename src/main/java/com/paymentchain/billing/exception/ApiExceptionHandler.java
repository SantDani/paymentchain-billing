...
import com.paymentchain.billing.common.StandaridzedApiExceptionResponse;
...
public class ApiExceptionHandler {   
    ...
    public ResponseEntity<StandaridzedApiExceptionResponse> handleBusinessRuleExceptions(BusinessRuleException ex) {
        StandaridzedApiExceptionResponse response = new StandaridzedApiExceptionResponse(ex.getCode(),"", ex.getMessage());      
       return new ResponseEntity<>(response,  ex.getHttpStatus() != null ? ex.getHttpStatus() : HttpStatus.INTERNAL_SERVER_ERROR);
     
    }  
    
    @ExceptionHandler(IOException.class)
    public ResponseEntity<StandaridzedApiExceptionResponse> handleNoContentException(IOException ex) {
        StandaridzedApiExceptionResponse response = new StandaridzedApiExceptionResponse("Input Ouput Error","erorr-1024",ex.getMessage());
